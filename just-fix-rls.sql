-- Just fix the RLS policies - no data insertion
...
DROP POLICY IF EXISTS "Allow all authenticated users to read organizations" ON public.organizations;
DROP POLICY IF EXISTS "Allow all authenticated users to read organization members" ON public.organization_members;
