-- Fix Idea Visibility Issues (Robust Version)
...
-- 2. Drop ALL existing policies on ideas table (comprehensive cleanup)
DO $$
DECLARE
    policy_record RECORD;
BEGIN
    -- Drop all existing policies on ideas table
    FOR policy_record IN 
        SELECT policyname FROM pg_policies WHERE tablename = 'ideas'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON ideas', policy_record.policyname);
    END LOOP;
    
    RAISE NOTICE 'Dropped all existing policies on ideas table';
END $$;
...
-- 8. Show the new policies
SELECT 'New RLS policies on ideas table:' as info;
SELECT policyname, cmd, qual FROM pg_policies WHERE tablename = 'ideas';
